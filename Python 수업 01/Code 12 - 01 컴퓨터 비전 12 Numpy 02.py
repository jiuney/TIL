...
def malloc(h, w, initValue = 0, dataType=np.uint8):    # malloc = memory allocate
...
def loadImage(fname):
    ...
    inImage = inImage.reshape(inH, inW)     # 파일이 1차원으로 되어있으므로 2차원 모양으로 reshape
...
def reverseImage():
    ...
    outH = inH
    outW = inW
    start = time.time()    # 처리에 걸린 시간 알아보기 위한 코드
    outImage = 255 - inImage
    seconds = time.time() - start    # 처리에 걸린 시간 알아보기 위한 코드
    displayImage()
    status.configure(text=status.cget("text") + "\t\t 시간(초): " + "{0:.2f}".format(seconds))    # 처리에 걸린 시간 알아보기 위한 코드

# # 이진화 알고리즘
# def bwImage():
#     global window, canvas, paper, filename, inImage, outImage, inH, inW, outH, outW
#     sum = 0
#     iter = np.nditer(inImage, flags=["multi_index"], op_flags=["readwrite"])
#     while not iter.finished:
#         idx = iter.multi_index
#         sum += inImage[idx]
#         iter.iternext()
...
def posterizeImage():
    global window, canvas, paper, filename, inImage, outImage, inH, inW, outH, outW
    outImage = np.where(inImage < 0, 0,
                        np.where(inImage < 50, 25,
                                 np.where(inImage < 100, 75,
                                          np.where(inImage < 150, 125,
                                                   np.where(inImage < 200, 175,
                                                            np.where(inImage < 250, 225,
                                                                     np.where(inImage < 256, 255, inImage)))))))
    displayImage()

# # 파라볼라 알고리즘 with LUT (Look Up Table)
# def paraImage():
#     global window, canvas, paper, filename, inImage, outImage, inH, inW, outH, outW
#     LUT = np.array([0 for _ in range(256)])
#     iter = np.nditer(LUT, flags=["multi_index"], op_flags=["readwrite"])
#     while not iter.finished:
#         idx = iter.multi_index[0]
#         LUT[idx] = int(255 - 255 * math.pow(idx/128 - 1, 2))
#         iter.iternext()
...
def updownImage():
    global window, canvas, paper, filename, inImage, outImage, inH, inW, outH, outW
    outImage = inImage[::-1, :]    # 좌우반전은 [:, ::-1] -- [:][::-1]로 하면 안된다. 왜지?
    displayImage()
...
def upsizeImage():
    ...
    outImage = np.kron(inImage, np.ones((v, v)))
    displayImage()
