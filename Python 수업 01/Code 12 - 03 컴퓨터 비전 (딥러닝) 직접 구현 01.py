...
#     for i in range(inH):
#         for k in range(inW):
#             inImage[R][i][k], inImage[G][i][k], inImage[B][i][k] = colorsys.rgb_to_hsv(inImage[R][i][k]/255, inImage[G][i][k]/255, inImage[B][i][k]/255)
#
#     for RGB in range(3):
#         # 원 입력 --> 임시 입력
#         for i in range(inH):
...
#         for i in range(outH):
#             for k in range(outW):
#                 outImage[RGB][i][k] = tmpOutImage[i][k]
#
#     # HSV --> RGB
#     for i in range(outH):
#         for k in range(outW):
#             outImage[R][i][k], outImage[G][i][k], outImage[B][i][k] = colorsys.hsv_to_rgb(outImage[R][i][k], outImage[G][i][k], outImage[B][i][k])
#             outImage[R][i][k], outImage[G][i][k], outImage[B][i][k] = round(outImage[R][i][k] * 255), round(outImage[G][i][k] * 255), round(outImage[B][i][k] * 255)
#
#     displayImageColor()    ####### 안돼서 포기 ㅠㅠ
...
comVisionMenu4.add_command(label="엠보싱 처리", command=embossImageColor)
