...
def displayImage():
    ...
    #         paper.put("#%02x%02x%02x" % (r, g, b), (k, i))    # (k, i)는 넣을 위치를 지정해주는 것 -- (i,k)가 아니고 (k,i)인 이유는 데이터 저장 상태가 기울어져 있으므로 제대로 표시되려면 위치를 바꿔줘야 하므로 이렇게 함.
    # 성능 개선
...
def rotateImage2():
    ...
    for i in range(inH):
        for k in range(inW):
            xs = i
            ys = k
            xd = int(((xs - cx) * math.cos(radian))-(((hy - ys) - cy) * math.sin(radian))+ cx)
            yd = int(((xs - cx) * math.sin(radian)) + (hy - (((hy - ys) - cy) * math.cos(radian))) + cy)
            if 0 <= xd < inH and 0<= yd < inW:
                outImage[xd][yd] = inImage[i][k]
    displayImage()
    ########################### 미치겠다ㅠㅠ
